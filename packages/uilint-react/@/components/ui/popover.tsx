...
function usePopoverLogic(controlledOpen?: boolean, onOpenChange?: (open: boolean) => void) {
...
export function PopoverRoot({ children, className, open, onOpenChange }: PopoverRootProps) {
...
export function PopoverTrigger({ children, className, style, disabled, title, "aria-label": ariaLabel }: PopoverTriggerProps) {
  ...
  return (
    <motion.button
      ...
      className={cn(
        "flex h-9 items-center border border-zinc-200 border-zinc-950/10 bg-white px-3 text-zinc-950 dark:border-zinc-50/10 dark:bg-zinc-700 dark:text-zinc-50 dark:border-zinc-800",
        className
      )}
      style={{
        borderRadius: 8,
...
export function PopoverContent({ children, className, portal, style, "data-ui-lint": dataUILint }: PopoverContentProps) {
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          layoutId={`popover-${uniqueId}`}
          className={cn(
            "absolute h-[200px] w-[364px] overflow-hidden border border-zinc-200 border-zinc-950/10 bg-white outline-none dark:bg-zinc-700 z-50 dark:border-zinc-800", // Changed z-90 to z-50
            className
          )}
          style={{
            borderRadius: 12,
...
export function PopoverSubmitButton({ className }: PopoverSubmitButtonProps) {
  return (
    <button
      className={cn(
        "relative ml-1 flex h-8 shrink-0 scale-100 select-none appearance-none items-center justify-center rounded-lg border border-zinc-200 border-zinc-950/10 bg-transparent px-2 text-sm text-zinc-500 transition-colors hover:bg-zinc-100 hover:text-zinc-800 focus-visible:ring-2 active:scale-[0.98] dark:border-zinc-50/10 dark:text-zinc-50 dark:hover:bg-zinc-800 dark:border-zinc-800",
        className
      )}
      type="submit"
