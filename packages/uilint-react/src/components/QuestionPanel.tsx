...
/**
 * Builds an updated style guide markdown string from existing content and user answers
 */
function buildUpdatedStyleGuide(
  ...
  // Preserve other colors from existing content
  const existingColors = extractSection(existingContent, "Colors");
  existingColors
    .filter((line) => !line.includes("**Primary**"))
  ...
  if (answers["font-weights"]) {
    ...
    lines.push(
      `- **Font Weights**: ${
        weightMap[answers["font-weights"]] || answers["font-weights"]
      }`
    );
  }
  // Preserve other typography from existing content
  const existingTypography = extractSection(existingContent, "Typography");
  existingTypography
    .filter((line) => !line.includes("**Font Weights**"))
    .forEach((line) => lines.push(line));
  ...
  // Preserve other spacing from existing content
  const existingSpacing = extractSection(existingContent, "Spacing");
  existingSpacing
    .filter((line) => !line.includes("**Base unit**"))
    .forEach((line) => lines.push(line));
  ...
  // Preserve other sections from existing content
